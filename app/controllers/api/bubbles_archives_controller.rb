class Api::BubblesArchivesController < ApplicationController
  def index
  end
  def show
  end
  def create
    ...
    for i in 0...bubble_num do
      ...
      if @bubbles_archive.save
        bubble_ids.push(@bubbles_archive.id)
      else
        render json: @bubbles_archive.errors, status: :unprocessable_entity
      end
    end
    render json: bubble_ids, status: :ok
  end

  def fetch_bubbles_archives
    ids = params[:ids]
  ...
  def bubbles_archive_params
    params.permit(bubbles_archives: [
      bubbles_archive: [:header,:image,:body,:footer]
    ])
  end
end